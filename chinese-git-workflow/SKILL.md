...
description: 国内 Git 平台配置参考——Gitee、Coding.net、极狐 GitLab、CNB 的 SSH/HTTPS/凭据/CI 接入差异与镜像同步配置。当用户要求配置国内 Git 平台（Gitee/Coding.net/极狐 GitLab/CNB）SSH/HTTPS/凭据/CI、或做镜像同步时调用；用户未明确提出国内 Git 平台配置需求时不自动触发。
version: "1.0.0"
license: MIT
metadata:
  hermes:
...
采用约定式提交（Conventional Commits）中文版，完整的类型定义、模板、subject/body 规范、Breaking Change 标注、Issue 关联、commitlint/husky 配置及检查清单，参考 **superpowers:chinese-commit-conventions**。

### 类型 emoji 速查（可选，团队自选是否启用）
