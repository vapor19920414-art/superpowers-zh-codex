...
**按需参考：** 开始读取附件、生成/修订报告或从报告转入修复前，必须阅读 [references/analysis-workflow.md](references/analysis-workflow.md)。需要隔离代码工作区时，必须先取得用户确认，再使用 `using-git-worktrees`；复杂修复先使用 `writing-plans`。
...
- **工作区选择门禁**：从报告转入修复时，若用户本次指令未明确指定“当前检出 / 复用指定 worktree / 新建 worktree”，必须先询问并取得确认。只有用户明确选择 worktree 后，才能使用 `using-git-worktrees` 或执行 `git worktree add`；不得因当前工作区不适合、多仓结构或历史报告已绑定工作区而自行创建或切换 worktree。
- **工作区复用门禁**：用户明确选择复用报告绑定或其他既有 worktree 后，必须先校验再原地复用；失效或冲突时等待用户决定，禁止静默创建第二个工作区或改到其他工作区。
- **工作区生命周期门禁**：只读分析默认不创建 worktree；仅在用户明确确认后创建，并在报告记录“本任务创建/既有工作区”。交付前审计本任务创建的 worktree：仅当工作树干净、分支无独有提交、预期成果已进入目标仓库时，用 `git worktree remove` 和 `git branch -d` 收尾；否则保留并报告原因。多仓项目必须按报告列出的业务子仓从内到外逐层审计，顶层干净不代表嵌套仓干净。禁止批量 `prune`、删除既有/归属不明 worktree，或用强制删分支绕过检查。
...
- “报告没问题，在当前目录按快速流程修复”——明确选择当前检出，不得自行改用 worktree。

两种修复模式都必须先取得报告认可。修复指令已明确当前目录、原 worktree 或新建 worktree 时按指定执行；未明确时先询问工作区选择。复杂度不确定时走完整流程；两者均不自动 commit、push、部署、改 TB 状态或发评论。
