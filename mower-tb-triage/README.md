...
完整流程用于跨模块、协议/状态机、安全相关或预计超过 5 个文件的改动；快速流程只用于边界明确的单模块最小修改。不确定时走完整流程。修复指令未明确当前目录、复用指定 worktree 或新建 worktree 时，必须先询问并取得选择；只有用户明确选择 worktree 后才能创建、复用或切换。
