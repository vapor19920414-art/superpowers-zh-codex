#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""从 mower-log-analysis 的分析报告，切分生成「TB 评论草稿」骨架。

草稿格式对齐 logs_auto/DLT_评论草稿.md 的五字段：根因 / 机制 / 证据 / 置信度 / 修复。
报告按 mower-log-analysis 的模板组织（## 0 TL;DR / ## 2 证据 / ## 3 根因 / ## 5 修复），
本脚本做机械切分映射；**它不重新总结**——精炼由人工（或 Claude）完成。
输出顶部会标注「待人工精炼」。

用法：
  python3 tb_draft.py /home/tudou/work/log/DLT-9/DLT-9_分析报告.md   # 打印到 stdout
  python3 tb_draft.py /home/tudou/work/log/DLT-9                      # 传目录，自动找报告
