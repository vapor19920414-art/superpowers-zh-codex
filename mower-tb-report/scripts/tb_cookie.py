...
用法：
  ...
  python3 tb_cookie.py --domain tb.orbbec.com --out /tmp/.tb_cookie
...
def main():
    ...
    ap.add_argument("--profile", default=None,
                    help="Chrome profile 名（省略时用配置；配置不存在且仅有一个 profile 时自动回退）")
    ...
    configured_profile = cfg.get("chrome_profile", "Profile 1")
    explicit_profile = args.profile is not None
    args.profile = args.profile or configured_profile
    src = os.path.join(args.base, args.profile, "Cookies")
    if not os.path.exists(src):
        avail = list_profiles(args.base)
        if not explicit_profile and len(avail) == 1:
            old_profile = args.profile
            args.profile = avail[0]
            src = os.path.join(args.base, args.profile, "Cookies")
            if os.path.exists(src):
                print(
                    f"[warn] 配置 profile {old_profile!r} 不可用；"
                    f"仅发现 {args.profile!r}，本次自动回退。",
                    file=sys.stderr,
                )
        if not os.path.exists(src):
